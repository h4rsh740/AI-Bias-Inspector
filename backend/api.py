...
async def get_rejection_tips(applicant_data: dict, shap_importance: list):
    """Generate personalized tips for rejected applicants using Gemini AI."""
    if not GEMINI_API_KEY:
        return [
            "Maintain a stable source of income.",
            "Avoid taking multiple loans simultaneously.",
            "Ensure all previous loan repayments were timely."
        ]
    
    try:
        model = genai.GenerativeModel('gemini-pro')
        # Filter SHAP results for negative impact features
        negative_features = [s['feature'] for s in shap_importance if s['value'] < 0][:3]
        
        prompt = f"""
        A loan applicant was rejected. Here is their profile:
        {applicant_data}
        
        The main features that negatively impacted the decision are: {negative_features}
        
        Provide 3 concise, highly actionable tips (max 15 words each) to help this specific applicant improve their profile for a future application. 
        Focus on the negative factors. 
        Format as a simple list of 3 bullet points.
        """
        
        response = model.generate_content(prompt)
        tips = [line.strip("- ").strip("* ").strip() for line in response.text.split('\n') if line.strip()][:3]
        return tips
    except Exception as e:
        print(f"Gemini Error: {e}")
        return ["Consult a financial advisor.", "Review your credit report.", "Reduce existing debts."]
...
@app.get("/")
async def root():
    return {"status": "FairLoan AI API is Live", "timestamp": time.time()}

# Local logging
def log_prediction(prediction_data: dict):
    """
    Log prediction event locally to stdout.
    """
    print(f"📝 [Prediction Logged] Prediction ID: {prediction_data.get('timestamp')} - Approved: {prediction_data.get('approved')}")
...
async def api_predict_loan(applicant: LoanApplicant, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Log prediction event in background
        log_data = {
            "timestamp": time.time(),
        ...
        background_tasks.add_task(log_prediction, log_data)

        # If rejected, get tips from Gemini
        tips = []
        if not prediction["approved"]:
            tips = await get_rejection_tips(applicant_dict, shap_results)
