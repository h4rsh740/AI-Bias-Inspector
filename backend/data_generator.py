...
def generate_dataset(n_samples: int = N_SAMPLES, seed: Optional[int] = 42) -> pd.DataFrame:
    ...
    for _ in range(n_samples):
        ...
        rows.append([income, age, gender, credit_score, loan_amount, employment_years, loan_approved])

    return pd.DataFrame(rows, columns=[
        "income", "age", "gender", "credit_score",
        "loan_amount", "employment_years", "loan_approved"
    ])
