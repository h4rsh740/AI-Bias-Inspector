...
export default function Dashboard() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const message = err.response ? `Server Error: ${err.response.status}` : `Connection Error: Check if Backend is live`;
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    {simulationResult ? (
                      ...
                      >
                        ...
                        {/* Gemini Tips */}
                        {!simulationResult.approved && simulationResult.rejection_tips && simulationResult.rejection_tips.length > 0 && (
                          <motion.div 
                          ...
                          >
                            <h4 style={{ fontSize: '14px', fontWeight: '700', color: '#4f8ef7', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                              💡 Gemini AI Recommendations
                            </h4>
                            <ul style={{ paddingLeft: '18px', margin: 0 }}>
                              {simulationResult.rejection_tips.map((tip, idx) => (
                                <li key={idx} style={{ fontSize: '13px', color: 'rgba(255,255,255,0.85)', marginBottom: '8px', lineHeight: '1.4' }}>
                                  {tip}
                                </li>
                              ))}
                            </ul>
